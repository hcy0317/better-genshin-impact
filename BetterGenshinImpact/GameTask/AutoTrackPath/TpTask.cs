...
public class TpTask
{
    ...
    private static string? s_lastSuccessfulTeleportCountry;
    ...
    private async Task<(double, double)> TpOnce(double tpX, double tpY, RouteMapContext mapContext, bool force = false)
    {
        ...
        {
            s_lastSuccessfulTeleportCountry = null;
        }
        else if (!string.IsNullOrWhiteSpace(target.Country))
        {
            s_lastSuccessfulTeleportCountry = target.Country;
        }
    ...
    private async Task SwitchToTeleportTargetMap(TeleportTargetContext target)
    {
        ...
        {
            if (ShouldForceSwitchToTargetCountry(
                    s_lastSuccessfulTeleportMapName,
                    s_lastSuccessfulTeleportCountry,
                    target.MapName,
                    target.Country))
            {
    ...
    internal static bool ShouldForceSwitchToTargetCountry(
        string? lastMapName,
        string? lastCountry,
        string targetMapName,
        string? targetCountry)
    {
        if (!string.Equals(targetMapName, MapTypes.Teyvat.ToString(), StringComparison.Ordinal) ||
            string.IsNullOrWhiteSpace(targetCountry))
        {
            return false;
        }

        return !string.IsNullOrEmpty(lastMapName) &&
               (!string.Equals(lastMapName, MapTypes.Teyvat.ToString(), StringComparison.Ordinal) ||
                !string.IsNullOrWhiteSpace(lastCountry) &&
                !string.Equals(lastCountry, targetCountry, StringComparison.Ordinal));
    }
    ...
    private async Task<bool> WaitForTeleportPanelAndConfirm(GiTpPosition? targetTp)
    {
        ...
        await Delay(TeleportPanelInitialDelayMs, ct);
        ...
        int clickAttempt)
    {
        ...
        while (stopwatch.ElapsedMilliseconds < MapChooseCandidateStableTimeoutMs)
        {
            ...
            Logger.LogInformation(
                ...
                stableChecks);
            await ClickMapChooseCandidate(stableImageRegion, currentCandidate);
