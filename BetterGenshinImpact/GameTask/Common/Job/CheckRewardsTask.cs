...
public class CheckRewardsTask
{
    ...
    private async Task StartOriginalAsync(CancellationToken ct)
    {
        ...
        await Delay(200, ct);
        await new ReturnMainUiTask().Start(ct);
        EnsureCommissionRewardsClaimed(done);
    }

    internal static void EnsureCommissionRewardsClaimed(bool? claimed)
    {
        if (claimed != true)
            throw new InvalidOperationException(claimed == false ? "每日委托奖励未领取" : "未确认每日委托奖励已领取");
    }

    /// <summary>
    /// 一条龙结束汇总：打开大地图识别原粹树脂、打开冒险之证识别委托奖励领取状态，
    /// 两块截图上下拼接后随通知发送。截图失败降级为文本；未确认领取仍沿用任务失败语义。
    /// </summary>
    private async Task StartSummaryAsync(CancellationToken ct)
    {
        ...
        try
        {
            ...
            EnsureCommissionRewardsClaimed(commissionClaimed);
        }
        finally
        {
            resinStrip?.Dispose();
