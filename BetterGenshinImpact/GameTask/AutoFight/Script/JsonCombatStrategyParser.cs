...
public static class JsonCombatStrategyParser
{
    ...
    public static JsonCombatStrategy ParseFile(string path, ILogger? logger = null)
    {
        var log = logger ?? NullLogger.Instance;
        if (!File.Exists(path))
        {
            log.LogError("JSON 战斗策略文件不存在：{Path}", path);
    ...
    /// <returns>解析后的战斗策略</returns>
    /// <exception cref="InvalidOperationException">解析失败或格式错误</exception>
    public static JsonCombatStrategy Parse(string json, ILogger? logger = null)
    {
        var log = logger ?? NullLogger.Instance;
        ...
        try
        {
            ...
            var version = document.Value<int?>("formatVersion") ?? 1;
            if (version != 1 || document["rotation"] != null) throw new JsonSerializationException("旧 rotation 原型已移除，请使用原策略 action 格式");
            strategy = document.ToObject<JsonCombatStrategy>(JsonSerializer.Create(new JsonSerializerSettings
            { MissingMemberHandling = MissingMemberHandling.Ignore, TypeNameHandling = TypeNameHandling.None }));
        }
        catch (JsonException ex)
        {
            log.LogError("JSON 战斗策略解析失败：{Msg}", ex.Message);
            throw new InvalidOperationException($"JSON 战斗策略格式错误：{ex.Message}", ex);
        }

        if (strategy == null)
        {
            log.LogError("JSON 战斗策略反序列化结果为空");
            throw new InvalidOperationException("JSON 战斗策略反序列化失败");
        }

        if (strategy.Info == null)
        {
            log.LogError("JSON 战斗策略缺少 Info 节点");
            throw new InvalidOperationException("JSON 战斗策略缺少 Info 节点");
        }

        if (strategy.Actions == null || strategy.Actions.Count == 0)
        {
            log.LogError("JSON 战斗策略缺少 Actions 节点或动作为空");
            throw new InvalidOperationException("JSON 战斗策略中未定义任何动作");
        }

        // 校验动作合法性（名称需能作为条件标识符解析；index 允许重复）
        ValidateActions(strategy.Actions, log);
    ...
    /// </summary>
    private static void ValidateActions(List<JsonAction> actions, ILogger log)
    {
        ...
        foreach (var action in actions)
        {
            ...
            {
                log.LogError("JSON 战斗策略中动作名称无法作为条件标识符解析（不能是布尔字面量、纯数字，不能含空白、逗号、运算符等，也不能与内置条件函数同名）：{Name}", action.Name);
