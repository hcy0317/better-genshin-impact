...
public partial class PathExecutor
{
    ...
    private async Task RecoverAtStatueAndRestartAsync(CaptureFrameStamp before)
    {
        ...
        if (!before.IsKnown || !CanRestartAfterHealing(CurWaypoints.Item2, resumeIndex))
            throw new InvalidOperationException("回血恢复缺少可安全回放的原传送入口；保留未完成，不重放路径宏");
        ...
        }, milliseconds => _moveIo.Delay(milliseconds, ct), ct, _moveIo.Clock);
    }
        ...
        Func<HealingFrame> observe, Func<int, Task> delay, CancellationToken ct, TimeProvider clock)
    {
        ...
        }, async milliseconds =>
        {
            ...
            throw new InvalidOperationException("神像恢复后未取得普通角色非低血新帧，禁止重启分段");
        ct.ThrowIfCancellationRequested();
        TaskExecutionScope.ThrowIfFailed();
        UiOperation.Current?.Check();
