...
这是新版配装工程的计算基础，不是完整配装界面或多人搜索器。它按需启动一个受限子进程，使用固定版本 gcsim 计算；不运行 BetterGI、原神或 RDP，不发送游戏输入，不修改装备或锁定状态。
...
本阶段只建立模拟承载与验证端口；角色/Enka 管理、多人优化、确认穿戴和战斗循环适配仍按已接受切片推进。实机验证与部署需要独立授权。
